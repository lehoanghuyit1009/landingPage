...
<%@ include file="/template/admin/inc/header-leftbar.jsp" %>
      ...
      <div id="content-wrapper">
              ...
              <div class="table-responsive">
                <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">
                	<div class="row justify-content-between">
	                	...
	                	<div class="col-sm-12 col-md-5">
	                		<form action="<%=request.getContextPath()%>/admin/news/search" method="get" class="d-flex input-group" id="search-form">
					        	<input type="text" class="form-control" value="<%=name%>" name="name" required placeholder="Search news" aria-label="Search" aria-describedby="basic-addon2" />
                	...
                	<div class="row">
                		<div class="col-sm-12">
		                	<table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%; table-layout: fixed;">
		                  		<thead>
		                    		<tr role="row">
		                    			<th class="sorting_asc text-center" width="15px" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending">
		                    				Id
		                    			</th>
		                    			<th class="sorting text-center" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending">
		                    				Name
		                    			</th>
		                    			<th class="sorting text-center" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending">
		                    				Description
		                    			...
		                    			<th class="sorting text-center" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending">
		                    				Action
		                    			</th>
		                    		</tr>
		                  		</thead>
		                  		<tbody>
		                  			...
		                  			<tr role="row" class="odd">
					                     <td class="text-center"><%=item.getId()%></td>
					                  	 ...
					                  	 <td class="text-center"><%=StringUtil.getText(item.getDescription(), 50)%></td>
					                  	 <td class="text-center"><%=item.getAddress()%></td>
					                  	 <td class="text-center"><%=item.getViews()%></td>
					                  	 <td class="text-center"><%=item.getCost()%></td>
					                  	 <td class="text-center"><%=item.getDateCreate()%></td>
					                    <td>
					                    	<div>
											<a href="<%=request.getContextPath()%>/admin/news/edit?page=<%=currentPage%>&id=<%=item.getId()%>" class="btn btn-info">
								          		<i class="fas fa-edit"></i></a>
											<a href="<%=request.getContextPath()%>/admin/news/del?page=<%=currentPage%>&id=<%=item.getId()%>" class="btn btn-danger"
		                    		...
		                    		%>
		                    		<tr><td colspan="8" style="text-align: center"><strong>Don't have news</strong></td></tr>
		                    		<%
		                  				}
		                    		%>
								</tbody>
               		...
               		<div class="row">
               			<div class="col-sm-12 col-md-5">
               				<div class="dataTables_info" id="dataTable_info" role="status" aria-live="polite">
               					<%
               						...
               						int to = (currentPage - 1) * DefineUtil.NUMBER_PER_PAGE + listNews.size();
               					%>
               					Showing <%=from%> to <%=to%> of <%=numberOfItems%> news.
               				</div>
               			</div>
               			<div class="col-sm-12 col-md-7">
               				<div class="dataTables_paginate paging_simple_numbers" id="dataTable_paginate">
               					<ul class="pagination">
	               					<%
	                              		String href = request.getContextPath() + "/admin/news/index?name="+name+"&page=";
	                              	%>
	                              	<!-- Xử lí nut previous -->
        ...
        <!-- code script here -->
        <script>document.getElementById('news').classList.add('active');</script>
