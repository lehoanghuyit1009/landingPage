...
<%@ include file="/template/public/inc/header.jsp" %>
		<%
			...
			if (request.getAttribute("numberOfPages") != null)
				numberOfPages = Integer.parseInt(request.getAttribute("numberOfPages").toString());
			if (request.getAttribute("numberOfItems") != null)
				numberOfItems = Integer.parseInt(request.getAttribute("numberOfItems").toString());
		%>
					...
					<div class="row">
						<div class="col-lg-8 post-list">
							...
							<div class="latest-post-wrap">
								...
								<h4 class="cat-title"><%=numberOfItems%> result for "<%=request.getParameter("name")%>"</h4>
								<%
								...
								<div class="single-latest-post row align-items-center">
									...
									<div class="col-lg-7 post-right">
										<a href="<%=request.getContextPath()%>/detail/<%=StringUtil.makeSlug(itemNews.getName())%>-<%=itemNews.getId()%>">
											<%
												/* hight light search name */
												String displayName = StringUtil.getText(itemNews.getName(), 60).replace(request.getParameter("name")
														, "<span style='color: red'>" + request.getParameter("name") + "</span>");
											%>
											<h4><%=displayName%></h4>
										</a>
										<ul class="meta">
											<li><a href="#"><span class="lnr lnr-user"></span><%=listUserName.get(itemNews.getCreateBy())%></a></li>
											<li><a href="#"><span class="lnr lnr-calendar-full"></span><%=DateUtil.getDateFormat(itemNews.getDateCreate())%></a></li>
											<li><a href="#"><span class="lnr lnr-bubble"></span><%=listCountComment.get(itemNews.getId()) %> Comments</a></li>
										</ul>
										<p><%=StringUtil.getText(itemNews.getDescription(), 170)%></p>
									</div>
								</div>
								<%
										}
								...
								%>
								<div class="not-news">
									<span>"<%=request.getParameter("name")%>" not found items.</span>
								</div>
								<%
									}
								%>
								...
								<%
			                    	if (listNews != null && listNews.size() > 0) {
			                        	String href = request.getContextPath() + "/search?name=" + request.getParameter("name") + "&page=";
			                    %>
								<!-- pagination -->
